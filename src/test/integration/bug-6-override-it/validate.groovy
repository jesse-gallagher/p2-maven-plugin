...
String jarName = "org.mozilla.javascript_1.7.2.jar"
assert files.contains(jarName)

Jar jar = new Jar(new File(target, jarName));
assert Util.symbolicName(jar) == "org.mozilla.javascript"
assert Util.version(jar) == "1.7.2"

// verify source artifact
String sourceName = "org.mozilla.javascript.source_1.7.2.jar"
assert files.contains(sourceName)

Jar sourceJar = new Jar(new File(target, sourceName));
assert Util.symbolicName(sourceJar) == "org.mozilla.javascript.source"
assert Util.version(sourceJar) == "1.7.2"
